 ...
 ******************************************************************************/
/*
 * A state that handles how the singleplayer menu functions
 */
package game.states;
...
public class State_MENU_SINGLEPLAYER extends State {
	...
	protected void ProcessInput() {
		...
		if (GUI.isButtonDown(0)) {
			State_SINGLEPLAYER sp = null;
			sp = (State_SINGLEPLAYER) (GameState.State_SinglePlayer.GetState());
			sp.Reset();
			GameState.SwitchToState(GameState.State_Menu_SinglePlayer,
		...
		if (GUI.isButtonDown(1)) {
			...
			try {
				...
				if (path != null) {
					...
					State_SINGLEPLAYER sp = null;
					sp = (State_SINGLEPLAYER) (GameState.State_SinglePlayer
							.GetState());
					sp.LoadGameSave(g);
					sp.setCreateNewGame(false);
					GameState.SwitchToState(GameState.State_Menu_SinglePlayer,
							GameState.State_SinglePlayer);
