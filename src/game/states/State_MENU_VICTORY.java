 ...
 ******************************************************************************/
/*
 * A state that handles how the victory screen functions
 */
package game.states;
...
public class State_MENU_VICTORY extends State {
		...
		try {
			...
			BackGroundImage = TextureLoader
					.getTexture(
							"PNG",
							ResourceLoader
									.getResourceAsStream("res/Materials/GUI/Backgrounds/VictoryScreen.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	protected void ProcessInput() {
		...
		if (GUI.isButtonDown(0)) {
			GameState.SwitchToState(GameState.State_VictoryScreen,
					GameState.State_Menu);
		}
		if (GUI.isButtonDown(1)) {
			GameState.State_VictoryScreen.ExitState();
		}
